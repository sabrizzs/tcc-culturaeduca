...
#     Aqui usamos sim = 1 / (1 + d) * 100.
#     Ajuste se quiser outro mapeamento.
#     """
...
# ) -> pd.DataFrame:
#     """
#     Versão do algoritmo "llm" que NÃO calcula embeddings em memória.

#     Suposições:
#       - Os embeddings de df1 estão em `entrada_embeddings` (idx_df1, embedding).
#       - Os embeddings de df2 (CNEFE) estão em `cnefe_embeddings` (cod_unico, embedding).
#       - df2 possui a coluna `cod_unico_endereco`, que corresponde a `cnefe_embeddings.cod_unico`.

#     Para cada linha de df1 (idx_df1), fazemos:

#         SELECT c.cod_unico, c.embedding <-> e.embedding AS distancia
#         FROM entrada_embeddings e
#         JOIN cnefe_embeddings c ON TRUE
#         WHERE e.idx_df1 = {idx_df1}
#         ORDER BY distancia
#         LIMIT top_n;

#     e montamos o dicionário de saída padrão.
#     """
#     print('entrou no llm')
...
#             print(idx1)
#             print(df1.loc[idx1, cod_unico_endereco_entrada])
#             # idx1_int = int(idx1)

#             # Busca no Postgres os top_n vizinhos (CNEFE) para o embedding da entrada idx1
#             cur.execute(
#                 """
#                 SELECT c.cod_unico,
#                        c.embedding <-> e.embedding AS distancia
...
#                 LIMIT %s;
#                 """,
#                 (df1.loc[idx1, cod_unico_endereco_entrada], top_n),
#             )
...
#             for cod_unico_cnefe, distancia in candidatos:
#                 cod_unico_cnefe = str(cod_unico_cnefe)

#                 # Localiza índice de df2
#                 idx2 = cod_to_idx2.get(cod_unico_cnefe)
#                 if idx2 is None:
#                     # Se não encontrarmos a linha correspondente em df2, ignoramos este vizinho
#                     print(f"pulou {cod_unico_cnefe}")
#                     continue

#                 score_final = distancia_para_similaridade(distancia)

#                 # Neste modelo baseado apenas em embeddings, usamos a similaridade final
#                 # como "similaridade_logradouro"; número/bairro ficam None por enquanto.
#                 score_log = score_final
#                 score_num = None
#                 score_bairro = None
...
import pandas as pd
import psycopg2
from psycopg2.extras import execute_values  # pode nem estar usando, mas deixei
import torch
import hnswlib
from sentence_transformers import SentenceTransformer, util
